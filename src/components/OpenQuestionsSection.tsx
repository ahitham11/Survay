...
import { ChevronRightIcon, SendIcon, UserIcon } from 'lucide-react';
import { Person, Evaluation } from '../types';
import { OPEN_QUESTIONS } from '../data/surveyData';
  ...
  return (
    <motion.div
      ...
      <div className="mb-5 sm:mb-8">
        <h3 className="text-xl sm:text-2xl font-bold text-white mb-1.5 sm:mb-2 flex flex-wrap items-center gap-2 sm:gap-3">
          الأسئلة المفتوحة
          <span className="text-xs sm:text-sm font-normal px-2.5 sm:px-3 py-0.5 sm:py-1 bg-slate-800 text-slate-400 rounded-full border border-slate-700">
            اختياري
          </span>
        </h3>
        <p className="text-slate-400 text-sm sm:text-base">
          يرجى الإجابة بكل شفافية وموضوعية. هذه الإجابات تساعدنا في فهم الصورة
          الكاملة.
        </p>
      </div>
      ...
      <div className="space-y-4 sm:space-y-6 mb-8 sm:mb-12">
        ...
        <motion.div
          ...
          className="bg-slate-800 border border-slate-700 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg shadow-black/10 focus-within:border-amber-500/50 focus-within:ring-1 focus-within:ring-amber-500/50 transition-all">
            ...
            <div className="flex gap-3 sm:gap-4 mb-3 sm:mb-4">
              ...
              className="text-sm sm:text-lg font-medium text-slate-200 mt-0.5 sm:mt-1 cursor-pointer leading-relaxed">
            ...
            <div className="sm:pr-12">
              <textarea
              id={`q-${question.id}`}
              value={evaluation.openAnswers[question.id] || ''}
              onChange={(e) => onUpdate(question.id, e.target.value)}
              placeholder="اكتب إجابتك هنا..."
              className="w-full bg-slate-900/50 border border-slate-700 rounded-lg sm:rounded-xl p-3 sm:p-4 text-sm sm:text-base text-slate-200 placeholder-slate-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 min-h-[90px] sm:min-h-[120px] resize-y transition-shadow"
              dir="rtl" />
      ...
      <div className="flex flex-col-reverse sm:flex-row justify-between items-stretch sm:items-center gap-3 sm:gap-4 bg-slate-800 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-slate-700">
        ...
        <motion.button
          ...
          className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 text-white px-5 sm:px-8 py-2.5 sm:py-3 rounded-xl font-bold text-sm sm:text-lg shadow-lg shadow-green-600/25 transition-all">
